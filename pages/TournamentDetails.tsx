...
const TournamentDetails: React.FC<Props> = ({ tournament: initialTournament, user, onBack }) => {
  ...
  const handleUpdateScore = (setIdx: number, side: 1 | 2, delta: number) => {
    ...
    if (setWinnerBefore !== 0 && delta > 0) return; // Prevent increasing if won
    ...
    // Auto-advance if set just finished
        ...
        <div className="fixed inset-0 bg-[#0c1221] z-[999] flex flex-col animate-in fade-in duration-300 font-sans overflow-hidden">
          ...
          <div className="flex items-center justify-between px-10 py-8">
            ...
            <div className="text-center">
              <div className="flex flex-col items-center">
                {matchWinner ? (
                  <div className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-10 py-2 rounded-full mb-2 animate-pulse">
                    <span className="text-xs font-black uppercase tracking-[0.4em] italic">MATCH DECIDED</span>
                  </div>
                ) : (
                ...
                <div className="flex items-center justify-center space-x-2">
                  {currentScores.map((_, idx) => {
                    ...
                    return (
                      ...
                      >
                         <div className={`w-20 h-2 rounded-full mb-2 transition-all ${winner === 1 ? 'bg-indigo-500 shadow-[0_0_15px_rgba(79,70,229,0.5)]' : winner === 2 ? 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]' : isActive ? 'bg-white' : 'bg-slate-700'}`}></div>
          ...
          <div className="flex-grow flex items-center justify-center gap-10 p-10">
            ...
            {(() => {
              ...
              const side = isSwapped ? 2 : 1;
              const isSetWon = getSetWinner(currentScores[activeSetIndex], scoringMatch.pointsOption, scoringMatch.goldenPoint) === side;
              const isOtherWon = getSetWinner(currentScores[activeSetIndex], scoringMatch.pointsOption, scoringMatch.goldenPoint) === (side === 1 ? 2 : 1);
              ...
              return (
                <div className={`flex-1 h-full rounded-[4rem] border-2 flex flex-col items-center justify-center relative shadow-[0_0_80px_rgba(0,0,0,0.5)] transition-all duration-700 ${colorClass} ${isSetWon ? 'ring-4 ring-white/10' : ''}`}>
                  ...
                  <div className="relative z-10 text-center w-full px-10">
                    <h2 className={`text-5xl font-black text-white uppercase italic tracking-tighter mb-3 transition-opacity ${isOtherWon ? 'opacity-30' : 'opacity-100'}`}>{t1?.name}</h2>
                    ...
                    <div className="flex items-center justify-center space-x-12 relative">
                       <button onClick={() => handleUpdateScore(activeSetIndex, side, -1)} className="w-32 h-32 rounded-full bg-white/5 border border-white/10 text-slate-400 hover:text-white flex items-center justify-center text-5xl font-light transition-all active:scale-90">
                         —
                       </button>
                       <div className="relative">
                         <span className={`text-[280px] font-black text-white leading-none italic tabular-nums tracking-tighter drop-shadow-[0_45px_45px_rgba(0,0,0,0.8)] z-10 relative transition-all ${isOtherWon ? 'opacity-20 scale-90' : 'opacity-100'}`}>
                           {score}
                         </span>
                       ...
                       <button 
                         disabled={isSetWon || isOtherWon}
                         onClick={() => handleUpdateScore(activeSetIndex, side, 1)} 
                         className={`w-32 h-32 rounded-full flex items-center justify-center text-6xl font-light transition-all active:scale-90 shadow-[0_20px_50px_rgba(0,0,0,0.4)] text-white ${btnClass} disabled:bg-slate-800 disabled:opacity-20 disabled:cursor-not-allowed`}
                       >
                         +
                       </button>
                    </div>
            ...
            {(() => {
              ...
              return (
                <div className={`flex-1 h-full rounded-[4rem] border-2 flex flex-col items-center justify-center relative shadow-[0_0_80px_rgba(0,0,0,0.5)] transition-all duration-700 ${colorClass} ${isSetWon ? 'ring-4 ring-white/10' : ''}`}>
                  ...
                  <div className="relative z-10 text-center w-full px-10">
                    <h2 className={`text-5xl font-black text-white uppercase italic tracking-tighter mb-3 transition-opacity ${isOtherWon ? 'opacity-30' : 'opacity-100'}`}>{t2?.name}</h2>
                    <div className="flex items-center justify-center space-x-2 text-slate-500 mb-12">
                       <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3.005 3.005 0 013.75-2.906z" /></svg>
          ...
          <div className="px-10 py-10 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
               ...
               <button 
                 ...
                 className={`px-12 py-5 rounded-[2rem] font-black uppercase text-xs tracking-[0.2em] transition-all active:scale-95 leading-none shadow-2xl ${matchWinner ? 'bg-emerald-500 hover:bg-emerald-400 text-white shadow-emerald-500/30' : 'bg-[#4f46e5] hover:bg-indigo-500 text-white shadow-indigo-500/30'}`}
               >
                 {matchWinner ? 'Finalize Match' : 'Submit Update'}
               </button>
            </div>
          </div>
        </div>
